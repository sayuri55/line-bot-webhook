...
PROMPT = (
    ...
    "・生年月日：YYYY年M月D日の形式で記載する（例：1952年7月11日）。元号は西暦に変換する。変換式：昭和N年＝(1925＋N)年、平成N年＝(1988＋N)年、令和N年＝(2018＋N)年。手書きの年号の数字は1桁ずつ丁寧に読み取り、変換してから出力する（例：昭和28年→1925＋28＝1953年）。\n"
    ...
    "  ※姓と名の間に全角スペースを入れる。\n"
    "・客番：申込確認書・申込書の「お客様番号」「契約番号」欄を優先して抽出する。1桁ずつ正確に読み取ること。\n"
    "・容量：数字とアルファベットのみ（例：60A、6KW）。B・低圧・電灯・動力などは除く。\n"
    "・電力会社：電気代明細書または申込確認書に記載されている電力会社名を使用する。ただし以下の読み替えを行う：明細書の発行者が「つくば電気プラン」の場合は「地域創生」、「株式会社フォーバルテレコム」または「フォーバルテレコム」の場合は「フォーバル」と記載する。申込兼同意書のヘッダーや申込先会社名（U-POWERなど）は使わない。明細書・申込確認書がない場合のみ、申込兼同意書右側の「現在の電力会社」欄を使用する。なお、抽出した電力会社名が申込兼同意書の「現在の電力会社」欄の記載と整合しているか必ず確認し、相違がある場合は明細書・申込確認書の情報を優先すること。\n"
    "・適用月：各マッチング情報の地番（供給地点特定番号）と同じ番号が記載されている明細書を特定し、その明細書の検針日をYYYY年M月D日形式で記載する。検針日の記載がない場合は利用期間の終了日を使用する。拠点①の適用月は拠点①の供給地点特定番号が載っている明細書から、拠点②の適用月は拠点②の明細書から読み取ること。異なる拠点の明細書の日付を絶対に混同しないこと。\n"
